...
function drawHeader(doc, title) {
  ...
  doc.moveDown(0.3);
  doc.fontSize(12).font("Helvetica-Bold").text(ORGANIZATION.eventName);
  doc.font("Helvetica").text(`(${ORGANIZATION.eventCode})`);
...
function drawBillTo(doc, participant) {
  const startY = doc.y;
  doc.font("Helvetica-Bold").fontSize(11).text("To", 50, startY);
...
function drawPaymentAndContact(doc, paymentRef) {
  ...
  doc.font("Helvetica-Bold").fontSize(11).text("Payment Details", 50, y);
  doc.font("Helvetica-Bold").fontSize(11).text("Contact Info", 320, y);
  ...
  leftRows.forEach(([label, value]) => {
    doc.font("Helvetica-Bold").text(`${label}`, 50, leftY, { continued: true, width: 110 });
  ...
  rightRows.forEach((line) => {
    doc.font("Helvetica").text(line, 320, rightY);
    rightY += 14;
  });
...
export async function generateInvoicePdf({
  ...
  return createPdfBuffer((doc) => {
    ...
    doc.font("Helvetica").fontSize(10).text(`Date: ${formatDate(issuedAt)}`, { align: "right" });
    doc.font("Helvetica").fontSize(10).text(`Invoice No: ${invoiceNumber}`, { align: "right" });
    doc.moveDown(1);
    ...
    const totalY = doc.y;
    doc.font("Helvetica-Bold").fontSize(12).text("TOTAL AMOUNT DUE", 320, totalY);
    doc.text(`$ ${formatMoney(amount)}`, 470, totalY);

    drawPaymentAndContact(doc, invoiceNumber);

    doc.fontSize(9).fillColor("#666666").text("Please use the Payment Ref above when making a bank transfer.", 50, doc.y, {
      align: "center",
      width: 495,
    });
  });
...
export async function generateReceiptPdf({
  ...
  return createPdfBuffer((doc) => {
    ...
    doc.font("Helvetica").fontSize(10).text(`Date: ${formatDate(paidAt)}`, { align: "right" });
    doc.font("Helvetica").fontSize(10).text(`Receipt No: ${receiptNumber}`, { align: "right" });
    doc.moveDown(1);

    drawBillTo(doc, participant);

    doc.font("Helvetica-Bold").fontSize(11).text("Payment Received", 50, doc.y);
    doc.moveDown(0.5);
    ...
    const totalY = doc.y;
    doc.font("Helvetica-Bold").fontSize(12).fillColor("#16a34a").text("TOTAL PAID", 320, totalY);
    doc.text(`$ ${formatMoney(amount)}`, 470, totalY);

    doc.moveDown(2);
    doc.fillColor("#333333").font("Helvetica").fontSize(10).text(
